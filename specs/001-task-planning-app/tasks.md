...
- [ ] T001 Create solution file `Stride.sln` at repo root and the `src/` and `tests/` folder layout per plan.md
- [ ] T002 [P] Create `src/ZSLabs.Stride.Domain/ZSLabs.Stride.Domain.csproj` (.NET 10 class library, nullable reference types enabled)
- [ ] T003 [P] Create `src/ZSLabs.Stride.Persistence/ZSLabs.Stride.Persistence.csproj` (.NET 10) referencing Domain, with latest-stable `Microsoft.EntityFrameworkCore.Sqlite` and `Microsoft.EntityFrameworkCore.Design` packages
...
- [ ] T009 [P] Create enums `UserRole`, `TaskStatus`, `TaskPriority`, `SubtaskStatus` in `src/ZSLabs.Stride.Domain/Enums/`
- [ ] T010 [P] Create `User` entity in `src/ZSLabs.Stride.Domain/Entities/User.cs` (data + constructor only, no business logic)
- [ ] T011 [P] Create `Space` entity in `src/ZSLabs.Stride.Domain/Entities/Space.cs`
- [ ] T012 [P] Create `Task` entity in `src/ZSLabs.Stride.Domain/Entities/Task.cs`
- [ ] T013 [P] Create `Subtask` entity in `src/ZSLabs.Stride.Domain/Entities/Subtask.cs`
- [ ] T014 [P] Create `Comment` entity in `src/ZSLabs.Stride.Domain/Entities/Comment.cs` (exactly one of TaskId/SubtaskId set)

### Persistence (DbContext, configurations, migration, seed)

- [ ] T015 Create `StrideDbContext` in `src/ZSLabs.Stride.Persistence/StrideDbContext.cs` with `DbSet`s for all entities
- [ ] T016 [P] Create `IEntityTypeConfiguration<T>` for each entity in `src/ZSLabs.Stride.Persistence/Configurations/` (unique `User.Username`, unique `Space.Key`, `Comment` one-owner check constraint, cascade delete Space→Task→Subtask→Comment and Task→Comment)
- [ ] T017 Add global UTC read-side `DateTime` convention (re-tag materialized values as `DateTimeKind.Utc`) in `StrideDbContext.ConfigureConventions`
- [ ] T018 Create the initial EF Core migration in `src/ZSLabs.Stride.Persistence/Migrations/` and wire `DbContext` registration (SQLite connection) in the Persistence layer
- [ ] T019 [P] Add `Persistence.Tests` verifying cascade delete chain, unique constraints, and the UTC read convention in `tests/ZSLabs.Stride.Persistence.Tests/`

### Backend cross-cutting infrastructure

- [ ] T020 Register services, DbContext, cookie authentication scheme, CORS (allow `http://localhost:5173` with credentials), and authorization policies `AdminOnly` / `RegularOnly` in `src/ZSLabs.Stride.Api/Program.cs`
- [ ] T021 [P] Add password hashing wrapper over `PasswordHasher<User>` in `src/ZSLabs.Stride.App/Services/PasswordHashingService.cs` (+ unit test in `tests/ZSLabs.Stride.App.Tests/`)
- [ ] T022 [P] Add global error-handling / problem-details middleware and generic error contract in `src/ZSLabs.Stride.Api/`
- [ ] T023 Add admin user seeding (single seeded `Admin`) on startup in `src/ZSLabs.Stride.Persistence/` (idempotent) and invoke from `Program.cs`

### Frontend foundation

- [ ] T024 [P] Create typed API client base (fetch wrapper with credentials) and mirrored contract types in `src/ZSLabs.Stride.Web/src/api/`
- [ ] T025 [P] Create shared design tokens + Tailwind entry stylesheet in `src/ZSLabs.Stride.Web/src/styles/`
- [ ] T026 Create app router shell, `useAuth` context skeleton, and role-based protected-route guard in `src/ZSLabs.Stride.Web/src/` (redirects unauthenticated users; separates admin vs. regular areas)
- [ ] T027 [P] Add a local-time date formatting utility (UTC → viewer local via `Intl.DateTimeFormat`) in `src/ZSLabs.Stride.Web/src/utils/`
